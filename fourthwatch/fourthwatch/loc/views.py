	...
	def get(self,request ,id, *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if type(user.user_type) == auth_models.Customer:
				...
				data = models.LOC.objects.filter(Q(applicant=user) | Q(beneficiary=user),id=id)
				...
				data = models.LOC.objects.filter(applicant__in = _bank_customer,id=id)
				result_loc = list()
				for d in data:
				...
				data = models.LOC.objects.filter(beneficiary__in = _bank_customer,id=id)
		...
		if user.is_authenticated():
			...
			if type(user.user_type) == auth_models.Customer:
				...
				for d in data:
					...
					has_approved = models.Transaction.objects.filter(party=user,type="APPROVE")
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				...
				from_composer = class_loc.InitialLOC().create(instance.id,user.id,s.validated_data['beneficiary'].id,\
						rules,product,request.build_absolute_uri(instance.locFile.url),\
	...
	def post(self, request , *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				instance = s.save() 
				result['status'] = True
				result['result'] = class_loc.Approve().create(instance.loc.id,instance.party.id,instance.party)
				notification = dict()
	...
	def post(self, request , *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				...
				instance = s.save() 
				result['result'] = class_loc.Reject().create(instance.loc.id,instance.party.id,instance.party,closeReason)
				notification = dict()
	...
	def post(self, request , *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				...
				instance = s.save() 
				result['result'] = class_loc.ShipProduct().create(instance.loc.id,instance.party.id,instance.party,evidence)
				notification = dict()
	...
	def post(self, request , *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				result['status'] = True
				instance = s.save() 
				result['result'] = class_loc.ReceiveProduct().create(instance.loc.id,instance.party.id,instance.party)
				notification = dict()
	...
	def post(self, request , *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				instance = s.save() 
				result['status'] = True
				result['result'] = class_loc.ReadyForPayment().create(instance.loc.id,instance.party.id,instance.party)
				notification = dict()
				notification['text'] = "%s has Received product LOC document. Please Click here to review" % user.first_name
				notification['loc'] = instance.loc
				notification['notification_to'] = instance.loc.beneficiary
	...
	def post(self, request , *args , **kwargs):
		...
		if user.is_authenticated():
			...
			if s.is_valid():
				...
				s.validated_data.pop('closeReason')
				instance = s.save() 
				result['result'] = class_loc.PaymentReceived().create(instance.loc.id,instance.party.id,instance.party,closeReason)
				notification = dict()
